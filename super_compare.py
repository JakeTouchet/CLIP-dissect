...
parser.add_argument("--results_dir", type=str, default="../images", help="where to save result images")
...
parser.add_argument("--top_k", type=int, default=10, help="Number of top neurons to compare")
...
def compare_top_k(args, model1_data, model2_data, words, pil_data):
    fig1 = process_model(args, args.target_model1, *model1_data, words, pil_data, 
                         torch.sort(torch.max(model1_data[0], dim=1)[0], descending=True)[1][:args.top_k])
    fig2 = process_model(args, args.target_model2, *model2_data, words, pil_data, 
                         torch.sort(torch.max(model2_data[0], dim=1)[0], descending=True)[1][:args.top_k])
    
    return fig1, fig2

def compare_same_neurons(args, model1_data, model2_data, words, pil_data):
    top_neurons = torch.sort(torch.max(model1_data[0], dim=1)[0], descending=True)[1][:args.top_k]
    fig1 = process_model(args, args.target_model1, *model1_data, words, pil_data, top_neurons)
    fig2 = process_model(args, args.target_model2, *model2_data, words, pil_data, top_neurons)
...
def main(args):
    ...
    for target_model in [args.target_model1, args.target_model2]:
        ...
        model_data[target_model] = (similarities, target_feats)

    # Ensure results directory exists
    os.makedirs(args.results_dir, exist_ok=True)

    # Compare top-k neurons for each model
    fig1, fig2 = compare_top_k(args, model_data[args.target_model1], model_data[args.target_model2], words, pil_data)
    fig1.savefig(os.path.join(args.results_dir, f'{args.target_model1}_top_k_activations.png'), dpi=300, bbox_inches='tight')
    fig2.savefig(os.path.join(args.results_dir, f'{args.target_model2}_top_k_activations.png'), dpi=300, bbox_inches='tight')
    plt.close(fig1)
    plt.close(fig2)

    # Compare same neurons (top-k from first model)
    fig1, fig2 = compare_same_neurons(args, model_data[args.target_model1], model_data[args.target_model2], words, pil_data)
    fig1.savefig(os.path.join(args.results_dir, f'{args.target_model1}_same_neurons_activations.png'), dpi=300, bbox_inches='tight')
    fig2.savefig(os.path.join(args.results_dir, f'{args.target_model2}_same_neurons_activations.png'), dpi=300, bbox_inches='tight')
    plt.close(fig1)
    plt.close(fig2)
